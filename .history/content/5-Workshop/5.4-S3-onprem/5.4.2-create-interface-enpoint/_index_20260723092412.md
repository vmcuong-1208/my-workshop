  ...
  const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "You are a learning assistant. Return valid JSON only. Do not wrap JSON in markdown fences.",
        },
...
async function getUserLogs(userId, startDate, endDate, tags = []) {
  ...
  return (result.Items || []).filter((item) => {
    ...
    const hasTags =
      tags.length === 0 ||
      tags.some((tag) =>
        (item.tags || []).map((value) => String(value).toLowerCase()).includes(String(tag).toLowerCase())
      );
...
async function handleQuizJob(message) {
  const { userId, quizId, topic, questionCount = 5, difficulty = "medium", startDate, endDate, tags = [] } = message;
  ...
  const logs = await getUserLogs(userId, startDate, endDate, tags);
  const logsText = logs.map((log) => `${log.title}: ${log.content}`).join("\n\n");
